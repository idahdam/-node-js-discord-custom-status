...
const runDiscordFunction = async () => {
  const hitEventList = await axios.get(
    `${DISCORD_API_BASE_URL}/guilds/${DISCORD_API_GUILD_ID}/scheduled-events?${process.env.DISCORD_API_GUILD_PARAMS}`,
    {
      headers: {
  ...
  if (hitEventList.data.length > 0) {
    ...
    for (var i = 0; i < hitEventList.data.length; i++) {
      ...
      // check if event's day is equal to today
      if (
        eventStartTime.format("YYYY-MM-DD") == currentTime.format("YYYY-MM-DD")
      ) {
        ...
        if (
          ...
          moment(currentTime).format("HH:mm") <
            moment(earliest_time).format("HH:mm") &&
          moment(currentTime).format("HH:mm") <=
            moment(latest_time).format("HH:mm") &&
          running == false &&
...
const setDiscordMessage = async (
  message,
  eventEndTime = moment("2022-02-02 08:00:00")
) => {
  try {
    ...
    await axios.patch(
      `${DISCORD_API_BASE_URL}/users/@me/settings`,
      {
        custom_status: {
          text: finalMessage,
        },
